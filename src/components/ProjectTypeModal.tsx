...
export const ProjectTypeModal = ({ isOpen, onClose, onSelect }: ProjectTypeModalProps) => {
    const savedPrefs = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('velix_project_type') || '{}') : {};
    const [selectedPlatform, setSelectedPlatform] = useState<string>(savedPrefs.platform || 'minecraft');
    const [selectedCategory, setSelectedCategory] = useState<string>(savedPrefs.category || '');
    const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null);

    useEffect(() => {
        if (isOpen) {
            const prefs = JSON.parse(localStorage.getItem('velix_project_type') || '{}');
            if (prefs.platform) setSelectedPlatform(prefs.platform);
            const cats = categories[prefs.platform || 'minecraft'] || [];
            if (prefs.category && cats.some((c: any) => c.id === prefs.category)) {
    ...
    const handleConfirm = () => {
        if (selectedLanguage) {
            localStorage.setItem('velix_project_type', JSON.stringify({ platform: selectedPlatform, category: selectedCategory }));
